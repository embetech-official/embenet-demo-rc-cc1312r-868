...
static void customServiceReceptionHandler(EMBENET_UDP_SocketDescriptor const* socket, EMBENET_IPV6 const* sourceAddress, uint16_t sourcePort, void const* data, size_t dataSize) {
    ...
    if (dataSize >= 6) {
        ...
        if (0 == strncmp(msg, "led1on", 6)) {
            ...
            printf("CUSTOM_SERVICE: Led 4 on\n");
            GPIO_write(CONFIG_LED_3_GPIO, CONFIG_GPIO_LED_ON);
        } else if (0 == strncmp(msg, "led4off", 7)) {
            printf("CUSTOM_SERVICE: Led 4 off\n");
            GPIO_write(CONFIG_LED_3_GPIO, CONFIG_GPIO_LED_OFF);
        } else {
